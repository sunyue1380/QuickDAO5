...
public class DQLTest extends DataSourceParameterized {
    ...
    public void fetch() {
        ...
        }
        {
            ...
            Assert.assertTrue(dao.existAny(personList));
        }
    }
    ...
    public void groupBy() {
        {
            int cityCount = dao.query(Person.class)
    ...
    public void column() {
        ...
        {
            List<Person> personList = dao.query(Person.class)
                    .excludeColumn("createdAt", "updatedAt")
                    .execute()
                    .getList();
            Assert.assertEquals(3, personList.size());
            for (Person person : personList) {
        ...
        }
        {
            ...
            Assert.assertEquals("Wilson", personObject.containsKey("last_name")?personObject.getString("last_name"):personObject.getString("LAST_NAME"));
    ...
    public void select() {
        ...
        if("sqlite".equalsIgnoreCase(databaseName)
            ...
            password = "7199e4df92df94bb67252513b070945c";
        }
        JSONArray array = dao.rawSelect("select * from person where password = ?", password);
        Assert.assertEquals(3, array.size());
        array = dao.rawSelect("select * from person where last_name = ?", "Gates");
        Assert.assertEquals(1, array.size());
        JSONObject firstObject = array.getJSONObject(0);
        Assert.assertEquals("Gates", firstObject.containsKey("last_name")?firstObject.getString("last_name"):firstObject.getString("LAST_NAME"));
    }
}