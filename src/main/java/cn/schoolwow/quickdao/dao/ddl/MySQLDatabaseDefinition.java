...
public class MySQLDatabaseDefinition extends AbstractDatabaseDefinition {
    ...
    public boolean hasTable(String tableName) {
        String hasTableSQL = "show tables like '%" + tableName + "%';";
        return connectionExecutor.name("判断表是否存在").sql(hasTableSQL).executeAndCheckExists();
    }

    @Override
    public boolean hasColumn(String tableName, String columnName) {
        String hasTableColumnSQL = "select table_name, column_name, data_type, character_maximum_length, numeric_precision, is_nullable, column_key, extra, column_default, column_comment from information_schema.`columns` where table_name = '" + tableName + "' and column_name = '" + columnName + "';";
    ...
    public List<Property> getPropertyList(String tableName) {
        String getEntityPropertyListSQL = "select table_name, column_name, data_type, character_maximum_length, numeric_precision, is_nullable, column_key, extra, column_default, column_comment from information_schema.`columns` where table_schema = '" + getDatabaseName() + "'  and table_name = '" + tableName + "';";
    ...
    public Property getProperty(String tableName, String columnName) {
        String getEntityPropertyListSQL = "select table_name, column_name, data_type, character_maximum_length, numeric_precision, is_nullable, column_key, extra, column_default, column_comment from information_schema.`columns` where table_schema = '" + getDatabaseName() + "'  and table_name = '" + tableName + "' and column_name = '" + columnName + "';";
        Property property = new Property();
        connectionExecutor.name("获取表字段信息").sql(getEntityPropertyListSQL).executeQuery((resultSet) -> {
            if (resultSet.next()) {
                getProperty(resultSet, property);
    ...
    public boolean hasIndex(String tableName, String indexName) {
        String hasIndexSQL = "show index from " + quickDAOConfig.databaseProvider.escape(tableName) + " where key_name = '" + indexName + "';";
    ...
    public List<IndexField> getIndexField(String tableName) {
        String getIndexSQL = "select table_name, index_name, non_unique, column_name, index_type, index_comment from information_schema.`statistics` where table_schema = '" + getDatabaseName() + "' and table_name = '" + tableName + "';";
