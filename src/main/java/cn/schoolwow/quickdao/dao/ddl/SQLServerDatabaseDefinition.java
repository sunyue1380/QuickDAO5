...
public class SQLServerDatabaseDefinition extends AbstractDatabaseDefinition {
    ...
    public boolean hasTable(String tableName) {
        String hasTableSQL = "select name from sysobjects where xtype='u' and name = '" + tableName + "';";
        return connectionExecutor.name("判断表是否存在").sql(hasTableSQL).executeAndCheckExists();
    }

    @Override
    public boolean hasColumn(String tableName, String columnName) {
        String hasTableColumnSQL = "select table_name, column_name from information_schema.columns where table_name = '" + tableName + "' and column_name = '" + columnName + "';";
    ...
    public Entity getDatabaseEntity(String tableName) {
        Entity entity = new Entity();
        //获取所有表
        String getEntityListSQL = "select name from sysobjects where xtype='u' and name = '" + tableName + "' order by name;";
        connectionExecutor.name("获取表列表").sql(getEntityListSQL).executeQuery(resultSet -> {
            if (resultSet.next()) {
                entity.tableName = resultSet.getString("name");
                entity.properties = getPropertyList(entity.tableName);
        ...
        if (null != entity.tableName) {
            //获取表注释
            String getEntityCommentSQL = "select so.name table_name, isnull(convert(varchar(255),value),'') comment from sys.extended_properties ex_p left join sys.sysobjects so on ex_p.major_id = so.id where ex_p.minor_id=0 and so.name = '" + tableName + "';";
            connectionExecutor.name("获取表注释").sql(getEntityCommentSQL).executeQuery(resultSet -> {
                if (resultSet.next()) {
                    entity.comment = resultSet.getString("comment");
                }
    ...
    public List<Property> getPropertyList(String tableName) {
        String getEntityPropertyTypeListSQL = "select table_name,ordinal_position,column_name,data_type,is_nullable,character_maximum_length from information_schema.columns where table_name = '" + tableName + "';";
        List<Property> properties = new ArrayList<>();
        connectionExecutor.name("获取表字段类型信息").sql(getEntityPropertyTypeListSQL).executeQuery(resultSet -> {
            while (resultSet.next()) {
        ...
        String getPropertyCommentList = "select b.name table_name, c.name, convert(varchar(255),a.value) value from sys.extended_properties a, sysobjects b, sys.columns c where a.major_id = b.id and c.object_id = b.id and c.column_id = a.minor_id and b.name = '" + tableName + "';";
        ...
        //判断主键
        String getPrimaryKeyColumn = "select column_name from information_schema.key_column_usage where table_name = '"+tableName+"';";
        connectionExecutor.name("获取表主键").sql(getPrimaryKeyColumn).executeQuery(resultSet -> {
            if (resultSet.next()) {
                for (Property property : properties) {
    ...
    public Property getProperty(String tableName, String columnName) {
        Property property = new Property();
        //获取字段信息
        String getEntityPropertyTypeListSQL = "select table_name,ordinal_position,column_name,data_type,is_nullable,character_maximum_length from information_schema.columns where table_name = '" + tableName + "' and column_name = '" + columnName + "';";
        connectionExecutor.name("获取表字段类型信息").sql(getEntityPropertyTypeListSQL).executeQuery(resultSet -> {
            if (resultSet.next()) {
                property.column = resultSet.getString("column_name");
                property.columnType = resultSet.getString("data_type");
        ...
        //获取字段注释
        String getPropertyCommentList = "select b.name table_name, c.name, convert(varchar(255),a.value) value from sys.extended_properties a, sysobjects b, sys.columns c where a.major_id = b.id and c.object_id = b.id and c.column_id = a.minor_id and b.name = '" + tableName + "' and c.name = '" + columnName + "';";
        connectionExecutor.name("获取字段注释").sql(getPropertyCommentList).executeQuery(resultSet -> {
            if (resultSet.next()) {
                property.comment = resultSet.getString("value");
            }
    ...
    public boolean hasIndex(String tableName, String indexName) {
        String hasIndexSQL = "select name from sys.indexes WHERE object_id=OBJECT_ID('" + tableName + "', N'U') and name = '" + indexName + "';";
