...
public class H2DatabaseDefinition extends MySQLDatabaseDefinition {
    ...
    public boolean hasTable(String tableName) {
        String hasTableSQL = "select table_name from information_schema.tables where table_name = '" + tableName.toUpperCase() + "';";
        return connectionExecutor.name("判断表是否存在").sql(hasTableSQL).executeAndCheckExists();
    }

    @Override
    public boolean hasColumn(String tableName, String columnName) {
        String hasTableColumnSQL = "select table_name, column_name from information_schema.`columns` where table_schema = 'PUBLIC' and table_name = '"+tableName.toUpperCase()+"' and column_name = '"+columnName.toUpperCase()+"';";
    ...
    public List<Property> getPropertyList(String tableName) {
        String getTableColumnSQL = "select table_name, column_name, type_name, character_maximum_length, is_nullable, column_default from information_schema.`columns` where table_schema = 'PUBLIC' and table_name = '" + tableName + "';";
        List<Property> propertyList = new ArrayList<>();
        connectionExecutor.name("获取表字段列表").sql(getTableColumnSQL).executeQuery((resultSet) -> {
            while (resultSet.next()) {
    ...
    public Property getProperty(String tableName, String columnName) {
        String getTableColumnSQL = "select table_name, column_name, type_name, character_maximum_length, is_nullable, column_default from information_schema.`columns` where table_schema = 'PUBLIC' and table_name = '" + tableName.toUpperCase() + "' and column_name = '" + columnName.toUpperCase() + "';";
        Property property = new Property();
        connectionExecutor.name("获取表字段信息").sql(getTableColumnSQL).executeQuery((resultSet) -> {
            if (resultSet.next()) {
                getProperty(property, resultSet);
            }
        });
    ...
    public boolean hasIndex(String tableName, String indexName) {
        ...
        return connectionExecutor.name("判断索引是否存在").sql(hasIndexExistsSQL).executeAndCheckExists();
    }

    @Override
    public List<IndexField> getIndexField(String tableName) {
        String getIndexSQL = "select table_name, sql from information_schema.indexes where table_name = '" + tableName + "'";
