...
public class PostgreDatabaseDefinition extends AbstractDatabaseDefinition {
    ...
    public boolean hasTable(String tableName) {
        String hasTableSQL = "select tablename from pg_tables where schemaname='public' and tablename = '" + tableName + "';";
        return connectionExecutor.name("判断表是否存在").sql(hasTableSQL).executeAndCheckExists();
    }

    @Override
    public boolean hasColumn(String tableName, String columnName) {
        String hasTableColumnSQL = "select pg_class.relname as table_name, attname as column_name from pg_attribute join pg_class on pg_attribute.attrelid = pg_class.oid where attnum > 0 and atttypid > 0 and pg_class.relname = '" + tableName + "' and attname = '" + columnName + "';";
    ...
    public Entity getDatabaseEntity(String tableName) {
        Entity entity = new Entity();
        String getEntitySQL = "select relname as name,cast(obj_description(relfilenode,'pg_class') as varchar) as comment from pg_class c where relkind = 'r' and relname = '" + tableName + "' order by relname;";
        connectionExecutor.name("获取表列表").sql(getEntitySQL).executeQuery(resultSet -> {
            if (resultSet.next()) {
                entity.tableName = resultSet.getString("name");
                entity.comment = resultSet.getString("comment");
                entity.properties = getPropertyList(entity.tableName);
    ...
    public List<Property> getPropertyList(String tableName) {
        ...
        //提取默认值和主键信息
        String getEntityPropertyTypeListSQL = "select table_name,ordinal_position,column_name,column_default,is_nullable,udt_name,character_maximum_length,column_default from information_schema.columns where table_name = '" + tableName + "';";
    ...
    public Property getProperty(String tableName, String columnName) {
        ...
        //提取默认值和主键信息
        String getEntityPropertyTypeListSQL = "select table_name, ordinal_position,column_name,column_default,is_nullable,udt_name,character_maximum_length,column_default from information_schema.columns where table_name = '" + tableName + "' and column_name = '" + columnName + "';";
        connectionExecutor.name("获取表字段类型信息").sql(getEntityPropertyTypeListSQL).executeQuery(resultSet -> {
            if (resultSet.next()) {
                getProperty(resultSet, property);
            }
    ...
    public boolean hasIndex(String tableName, String indexName) {
        String hasIndexSQL = "select indexname from pg_indexes where tablename = '" + tableName + "' and indexname = '" + indexName + "';";
    ...
    public List<IndexField> getIndexField(String tableName) {
        String getIndexSQL = "select tablename,indexname,indexdef from pg_indexes where tablename = '" + tableName + "';";
