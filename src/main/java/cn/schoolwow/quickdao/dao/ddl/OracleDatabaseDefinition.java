...
public class OracleDatabaseDefinition extends PostgreDatabaseDefinition{
    ...
    public boolean hasTable(String tableName) {
        String hasTableSQL = "select table_name from user_tables where table_name = '" + tableName + "'";
        return connectionExecutor.name("判断表是否存在").sql(hasTableSQL).executeAndCheckExists();
    }

    @Override
    public boolean hasColumn(String tableName, String columnName) {
        String hasTableColumnSQL = "select table_name, column_name, data_type, nullable, data_length from user_tab_columns where table_name = '"+tableName+"'";
    ...
    public Entity getDatabaseEntity(String tableName) {
        String getEntitySQL = "select user_tables.table_name,user_tab_comments.comments from user_tables left join user_tab_comments on user_tables.table_name = user_tab_comments.table_name where user_tables.table_name = '"+tableName+"'";
        Entity entity = new Entity();
    ...
    public List<Property> getPropertyList(String tableName)  {
        String getEntityPropertyListSQL = "select table_name, column_name, data_type, nullable, data_length from user_tab_columns where table_name = '"+tableName+"'";
    ...
    public Property getProperty(String tableName, String columnName)  {
        String getEntityPropertyListSQL = "select table_name, column_name, data_type, nullable, data_length from user_tab_columns where table_name = '"+tableName+"' and column_name = '"+columnName+"'";
        Property property = new Property();
        connectionExecutor.name("获取表字段信息").sql(getEntityPropertyListSQL).executeQuery((resultSet)->{
            if(resultSet.next()){
                property.column = resultSet.getString("column_name");
    ...
    public boolean hasIndex(String tableName, String indexName) {
        String hasIndexSQL = "select index_name from user_indexes where table_name = '" + tableName + "' and index_name = '" + indexName + "'";
    ...
    public List<IndexField> getIndexField(String tableName){
        ...
        //获取索引信息
        String getIndexSQL = "select table_name, index_name,uniqueness from user_indexes where table_name = '"+tableName+"'";
